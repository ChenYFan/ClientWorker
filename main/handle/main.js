...
self.clientworkerhandle = async (request) => {
        ...
        for (let transform_rule of catch_rule.transform_rules) {
            ...
            switch (transform_rule.searchin || "url") {
                case "url":
                    if (
                        tReq.url.match(
                            new RegExp(
                                transform_rule.search,
                                transform_rule.searchflags
                            )
                        )
                    )
                        tSearched = true;
            ...
            switch (transform_rule.replacein || "url") {
                case "url":
                    ...
                    if (
                        typeof transform_rule.replace !== "undefined" &&
                        tSearched
                    ) {
                ...
                switch (transform_rule.action) {
                    case "fetch":
                        ...
                        if (typeof transform_rule.fetch === "undefined") {
                            cons.e(
                                `Fetch Config is not defined for ${tReq.url}`
                            );
                            break;
                        }
                        ...
                        tRes = await new Promise(async (res, rej) => {
                            const EngineFetcher = async () => {
                                ...
                                return new Promise(async (resolve, reject) => {
                                    if (!EngineFetch) {
                                        switch (
                                            transform_rule.fetch.engine ||
                                            "fetch"
                                        ) {
                                            case "fetch":
                                                cRes = await FetchEngine.fetch(
                                                    tReq,
                                                    fetchConfig
                                                );
                                                break;
                                            case "crazy":
                                                cRes = await FetchEngine.crazy(
                                                    tReq,
                                                    fetchConfig
                                                );
                                                break;
                                        ...
                                        switch (
                                            ...
                                            case "KFCThursdayVW50":
                                                ...
                                                cRes =
                                                    await FetchEngine.KFCThursdayVW50(
                                                        EngineFetchList,
                                                        fetchConfig
                                                    );
                                                break;
                                    ...
                                    if (
                                        typeof transform_rule.fetch.cache ===
                                            "object" &&
                                        cRes.status ===
                                        ...
                                        cache.match(tReq).then((cRes) => {
                                            if (!!cRes) {
                                                if (
                                                    ...
                                                    cons.s(
                                                        `${tReq.url} is fetched from cache`
                                                    );
                                                    res(cRes);
                                                } else {
                                                    cons.w(
                    ...
                    case "redirect":
                        if (typeof transform_rule.redirect === "undefined") {
                            cons.e(
                                `Redirect Config is not defined for ${tReq.url}`
                            );
                            break;
                        }
                    ...
                    case "return":
                        ...
                        return new Response(
                            transform_rule.return.body || "Error!",
                            {
                    ...
                    case "script":
                        if (typeof transform_rule.script === "undefined") {
                            cons.e(
                                `Script Config is not defined for ${tReq.url}`
                            );
                            break;
                        }
                        if (
                            typeof transform_rule.script.function === "string"
                        ) {
    ...
    if (!tFetched) {
        ...
        if (EngineFetch) {
            tRes = await FetchEngine.parallel(
                EngineFetchList,
                fetchConfig || {}
            );
        } else {
            tRes = await fetch(tReq.url, {
                method: tReq.method.match(/^(GET|HEAD|POST)$/g)
                    ? tReq.method
                    : "GET",
